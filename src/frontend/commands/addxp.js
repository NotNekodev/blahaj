...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('addxp')
        .setDescription('Adds XP for a user')
        .addIntegerOption(option =>
            option.setName('textxp')
                .setDescription('The amount of TextXP to add')
                .setRequired(true))
        .addUserOption(option =>
            option.setName('user')
                .setDescription('The user to add XP for')
                .setRequired(true))
    ...
    async execute(interaction, client) {
        ...
        await interaction.reply({ content: `Added TextXP to \`${textxp}\` for user <@${user.id}>`, allowedMentions: { parse: [] }, ephemeral: true });
    }
};
