...
function getUserConfig(userid) {
    return new Promise((resolve, reject) => {
        const db = getDB();
        if (!db) {
            resolve({ participate: true, pinged: true });
            return;
        }

        db.get('SELECT participate, pinged FROM users WHERE userid = ?', [userid], (err, row) => {
            if (err) {
                reject(err);
                return;
            }
            ...
            resolve({ participate: row.participate === 0 ? false : true, pinged: row.pinged === 0 ? false : true });
        });
    });
}

function setUserConfig(userid, { participate = null, pinged = null } = {}) {
    const db = getDB();
    ...
    return new Promise((resolve, reject) => {
        db.get('SELECT participate, pinged FROM users WHERE userid = ?', [userid], (err, row) => {
            ...
            const curPinged = row ? (row.pinged === 0 ? 0 : 1) : 1;

            const newParticipate = (participate === null) ? curParticipate : (participate ? 1 : 0);
            const newPinged = (pinged === null) ? curPinged : (pinged ? 1 : 0);

            db.run('INSERT INTO users (userid, participate, pinged) VALUES (?, ?, ?) ON CONFLICT(userid) DO UPDATE SET participate = ?, pinged = ?', [userid, newParticipate, newPinged, newParticipate, newPinged], function(runErr) {
                if (runErr) return reject(runErr);
                resolve({ participate: newParticipate === 1, pinged: newPinged === 1 });
            });
        });
    });
}
